...
.config(function($stateProvider, $urlRouterProvider) {
  ...
  .state('tabs.list', {
    ...
    views: {
      ...
      'fabContent': {
        template: '<button ng-click="go(1,9)" id="fab-profile" class="button button-fab button-fab-bottom-left button-positive"><i class="icon ion-model-s"></i></button>',
        controller: function ($scope, $http, $state) {
            $scope.go = function(item1, item2)
            {
              ...
              $http({
                ...
                $state.go('tabs.route',{location: data});
              }).error(function(err){
